...
export default function PowerAnalysisDashboard() {
  ...
  const handleFileUpload = (e) => {
    ...
    // Membaca sebagai ArrayBuffer agar aman untuk file ZTE (.xls / UTF-16 TSV) dan Hariff (.csv)
    reader.onload = (evt) => {
      const dataBuffer = new Uint8Array(evt.target.result);
      const wb = XLSX.read(dataBuffer, { type: 'array' });
      const wsname = wb.SheetNames[0];
      const ws = wb.Sheets[wsname];
      // Defval "" agar sel kosong tidak merusak struktur
  ...
  const parseAndAnalyzeData = (rawData) => {
    ...
    // Fungsi Pembantu untuk mencari kolom secara dinamis (Case Insensitive & Hapus Spasi)
  ...
  const calculateAnomalies = (dataset) => {
    const found = [];
    dataset.forEach(row => {
      // Abaikan nilai 0 mutlak (biasanya sensor mati sementara atau alat restart) kecuali Anda ingin menganggapnya 'Drop/Mati Total'
      
      // Hitung Anomali AC (Drop < 200V, Over > 240V)
      if (row.ac > 0 && (row.ac < 200 || row.ac > 240)) {
        found.push({ time: row.time, type: 'AC Mains', val: row.ac, status: row.ac < 200 ? 'Drop' : 'Overvoltage' });
      }
      // Hitung Anomali DC (Drop < 46V, Over > 54V)
  ...
  const handleSave = () => {
    ...
    alert(`Sukses! Data Historikal Site ${siteName} berhasil disimpan ke Vercel (Local Storage).`);
  };
